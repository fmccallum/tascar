...
class rec_vbaplutnull_t : public TASCAR::receivermod_base_speaker_t {
public:
  ...
  rec_vbaplutnull_t(tsccfg::node_t xmlsrc);
  virtual ~rec_vbaplutnull_t() {};
...
rec_vbaplutnull_t::data_t::data_t( uint32_t channels )
{
...
rec_vbaplutnull_t::data_t::~data_t()
{
  delete [] wp;
  delete [] dwp;
}

rec_vbaplutnull_t::rec_vbaplutnull_t(tsccfg::node_t xmlsrc)
...
*/
void rec_vbaplutnull_t::add_pointsource( const TASCAR::pos_t& prel,
                                  double width,
...
TASCAR::receivermod_base_t::data_t* rec_vbaplutnull_t::create_state_data(double srate,uint32_t fragsize) const
{
  return new data_t(spkpos.size());
}

REGISTER_RECEIVERMOD(rec_vbaplutnull_t);
