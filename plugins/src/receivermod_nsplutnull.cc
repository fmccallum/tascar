...
class nsplutnull_t : public TASCAR::receivermod_base_speaker_t {
public:
  ...
  nsplutnull_t( tsccfg::node_t xmlsrc );
  virtual ~nsplutnull_t(){ }
...
nsplutnull_t::data_t::data_t( uint32_t chunksize,uint32_t channels )
{
...
nsplutnull_t::nsplutnull_t( tsccfg::node_t xmlsrc )
...
void nsplutnull_t::add_variables( TASCAR::osc_server_t* srv )
{
  TASCAR::receivermod_base_speaker_t::add_variables( srv );
  srv->add_bool( "/useall", &useall );
}

void nsplutnull_t::add_pointsource( const TASCAR::pos_t& prel, double width, const TASCAR::wave_t& chunk, std::vector<TASCAR::wave_t>& output, receivermod_base_t::data_t* sd )
{
 

}

TASCAR::receivermod_base_t::data_t* nsplutnull_t::create_state_data( double srate,uint32_t fragsize ) const
{
  return new data_t( fragsize,spkpos.size() );
}

REGISTER_RECEIVERMOD( nsplutnull_t );
