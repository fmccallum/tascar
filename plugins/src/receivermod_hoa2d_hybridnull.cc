...
class hoa2d_hybridnull_t : public TASCAR::receivermod_base_t {
public:
  ...
  hoa2d_hybridnull_t(tsccfg::node_t xmlsrc);
  ~hoa2d_hybridnull_t();
...
hoa2d_hybridnull_t::data_t::data_t(uint32_t channels )
{
  B = std::vector<float>(channels, 0.0f );
  prevK = 0;
}

hoa2d_hybridnull_t::hoa2d_hybridnull_t(tsccfg::node_t xmlsrc)
...
hoa2d_hybridnull_t::~hoa2d_hybridnull_t()
{
  for (int i =0;i<diviser;i++){
...
void hoa2d_hybridnull_t::add_pointsource(const TASCAR::pos_t& prel, double width, const TASCAR::wave_t& chunk, std::vector<TASCAR::wave_t>& output, receivermod_base_t::data_t* sd)
{

}
void hoa2d_hybridnull_t::configure()
{
  n_channels = channels;
...
void hoa2d_hybridnull_t::postproc(std::vector<TASCAR::wave_t>& output){
...
void hoa2d_hybridnull_t::add_diffuse_sound_field(const TASCAR::amb1wave_t& chunk, std::vector<TASCAR::wave_t>& output, receivermod_base_t::data_t* sd)
{
  if( output.size() ){
...
TASCAR::receivermod_base_t::data_t* hoa2d_hybridnull_t::create_state_data(double srate, uint32_t fragsize) const
{
  return new data_t(channels);
}

REGISTER_RECEIVERMOD(hoa2d_hybridnull_t);
