...
class nspn_t : public TASCAR::receivermod_base_speaker_t {
public:
...
nspn_t::data_t::data_t( uint32_t chunksize,uint32_t channels )
{
...
nspn_t::nspn_t( tsccfg::node_t xmlsrc )
...
void nspn_t::add_variables( TASCAR::osc_server_t* srv )
{
  TASCAR::receivermod_base_speaker_t::add_variables( srv );
  srv->add_bool( "/useall", &useall );
}

void nspn_t::add_pointsource( const TASCAR::pos_t& prel, double width, const TASCAR::wave_t& chunk, std::vector<TASCAR::wave_t>& output, receivermod_base_t::data_t* sd )
{

  data_t* d( (data_t* )sd);
  TASCAR::pos_t psrc( prel.normal());

  //uint32_t kmin = static_cast<int>((psrc.azim()-spkpos[0].az+TASCAR_PI)/spacing);
...
TASCAR::receivermod_base_t::data_t* nspn_t::create_state_data( double srate,uint32_t fragsize ) const
{
  return new data_t( fragsize,spkpos.size() );
}

REGISTER_RECEIVERMOD( nspn_t );
